...
# Read the CSV file
df = pd.read_csv('phase1_integration_results.csv')
...
for index, row in df.iterrows():
    ...
    df.at[index, 'original_questionnairy'] = response
    status(counter, 70)
    counter+=1
...
for index, row in df.iterrows():
    ...
    df.at[index, 'decompiled_Ghidra_questionnairy'] = response
    status(counter, 70)
    counter+=1
...
df.to_csv('phase1_integration_results.csv', index=False)